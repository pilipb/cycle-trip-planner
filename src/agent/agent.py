...
def get_route(
    ...
    print(f"Getting route from {start_lon}, {start_lat} to {end_lon}, {end_lat}")
    """Get cycling route details between two coordinate points. Coordinates are in longitude, latitude order (e.g. Berlin: 13.4050, 52.5200)."""
...
def get_elevation_profile(origin: str, destination: str) -> ElevationResult:
    print(f"Getting elevation profile from {origin} to {destination}")
...
def get_weather(location: str, month: str) -> WeatherResult:
    print(f"Getting weather for {location} in {month}")
...
def find_accommodation(location: str, type: AccommodationType) -> list[Accommodation]:
    print(f"Finding accommodation for {location} of type {type}")
...
def get_points_of_interest(location: str, category: POICategory) -> list[PointOfInterest]:
    print(f"Getting points of interest for {location} of category {category}")
...
def check_visa_requirements(passport_country: str, destination_country: str) -> VisaResult:
    print(f"Checking visa requirements for {passport_country} to {destination_country}")
...
def estimate_budget(
    ...
    print(f"Estimating budget for {distance_km}km over {days} days in {country} with {accommodation_type}")
